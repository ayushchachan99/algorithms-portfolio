...
translation_table = str.maketrans({c: " " for c in string.punctuation})
...
def get_words_from_string(line):
    ...
    """
    line = line.translate(translation_table).translate(_upper_to_lower)
...
def word_frequencies_for_file(filename):
    ...
    print(
        f"File {filename}: {len(line_list)} lines, {len(word_list)} words, {len(freq_mapping)} distinct words"
    )
